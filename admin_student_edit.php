...
$strd = "SELECT * FROM student Where id='$eid' ";
...
if (isset($_POST['save'])) {
  ...
  $st12 = "SELECT * FROM student Where email='$email' and phone='$phn'";
  $r1 = mysqli_query($conn, $st12);
  $rowcount = mysqli_num_rows($r1);

  if ($rowcount == 0 || ($email == $r['email'] && $phn == $r['phone'])) {
    $strq = "UPDATE student SET name='$name',id=$id,department='$dep',email='$email',password= '$pass',gender='$gender',birthday='$bod',address='$add',phone='$phn' Where id='$eid'  ";
  ...
  <nav class="navbar navbar-expand-lg navbar-light bg-light">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link" style="color:blue;" href="https://localhost/REGISTRATION/student_list.php">Student List</a>
        </li>
        <li class="nav-item">
    ...
    <form method="POST" action="">
      <table style="text-align:center;" class="table table-success table-striped" class="table table-striped">
        ...
        <tbody>
          ...
          <tr>
            <td> ID </td>
            <td> <input disabled style="text-align:center; background-color: #b8d6cf" required="required" type="text" class="form-control" name="id" value="<?php echo $r['id']; ?>"><input hidden style="text-align:center; background-color: #b8d6cf"  type="text" class="form-control" name="id" value="<?php echo $r['id']; ?>"></td>
          </tr>
          <tr>
            <td> Email </td>
